...
def run(args):
    ...
    if (r_amount or g_amount or b_amount) and amount:
        logger.error(
            "error: cannot set individual colors and total amount simultaneously. exiting..."
        )
        exit(1)
    ...
    if args.doom_theme:
        doom_theme_file = Path(
            Path.home(), "dotfiles", ".doom.d", "themes", f"{args.theme}-theme.el"
        )
        ...
        for idx, line in enumerate(data):
            if '("#' in line:
                ...
                secondary_hex_color = line[
                    secondary_hash_idx + 3 : secondary_hash_idx + 9
                ]
                ...
                new_secondary_hex_color = (
                    f"{new_secondary_r}{new_secondary_g}{new_secondary_b}"
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="doombd", description="brighten or darken doom themes"
    )

    parser.add_argument("-o", "--op", help="the operation to use, e.g., + or -")
    parser.add_argument(
        "--amount", help="the amount to increase or decrease for all colors"
    )
    ...
    parser.add_argument(
        "--doom-theme",
        action="store_true",
        help="set to enable changing colors for selected doom theme",
    )
    parser.add_argument(
        "--theme", help="set to enable changing colors for selected theme"
    )
    parser.add_argument(
        "--alacritty",
        action="store_true",
        help="set to enable changing colors for alacritty",
    )
    parser.add_argument(
        "--k9s", action="store_true", help="set to enable changing colors for k9s skin"
    )
